...
本次版本针对上一版出现的“登录成功但会话保存失败、反复重登、请求超时”和写入后状态不确定问题进行了集中修复：

- 修复 Windows 凭据管理器无法保存超长 session token 的问题：短 token 继续使用系统凭据库，超长 token 自动使用本机 AES-256-GCM 加密文件，并兼容读取、删除和短长 token 切换。
- 修复并发登录、退出登录和迟到响应造成的 token 覆盖；退出会取消未完成登录，认证失败恢复不会重复打开浏览器，存储失败会暂停自动重登。
- 将请求超时扩展到响应体读取、限流退避和重试全过程；写请求不自动重放，HTML、非 JSON 或网络中断不会泄露内容。
- 修复业务接口只看 HTTP 200 或文本中出现 token 的误判；现在同时检查 HTTP 状态和业务状态，发现接口错误不会伪装成空数据。
- 增加采购草稿分页完整性、商品单位/数量/价格、重复订单和删除后的严格回查；无法唯一确认时返回“不确定”。
- 台账、晨检、委员会、设备、废弃物和票证写入均按提交字段回查。晨检改为必须传入逐员工实际记录，不再生成体温、时间或默认“正常”结果。
- 确认句柄绑定当前 Session 和认证版本；认证状态变化后自动失效，避免把旧确认用于新会话。
- 配置向导增加并发复用、重复提交保护、已有配置和别名保留，以及启动失败、窗口关闭、超时和半包请求的资源清理。
- 补充合成回归测试，当前本地共 78 项测试通过，覆盖跨进程锁、token 迁移、认证竞态、业务回查和敏感信息脱敏。

升级后请重启所有连接器进程；Windows 超长 token 的实际系统凭据行为仍需在目标 Windows 环境中验证。
